...
public class ExecutorInterceptor extends AbstractInterceptor {
	...
	private void processIntercept(final Object[] queryArgs) {
		...
		if (sql.toLowerCase().endsWith("for update")) {
			...
			if ("db2dialect".equals(this.dialect.getClass().getSimpleName().toLowerCase())) {
				// for update with rs
				sql += " with rs";
			} else if ("oracledialect".equals(this.dialect.getClass().getSimpleName().toLowerCase())) {
				// for update
			} else if ("mysqldialect".equals(this.dialect.getClass().getSimpleName().toLowerCase())) {
				// for update
			}
