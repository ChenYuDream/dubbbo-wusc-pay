...
public class CmbcOnlineRemitImpl implements RemitBankOnlineService {
	...
	public void BankOnlineExport(File file, List<RemitProcess> remitProcessList, RemitChannel remitChannel) {
		...
		for (RemitProcess remitProcess : remitProcessList) {
			if (("305".equals(remitProcess.getBankChannelNo().substring(0, 3)) && remitProcess.getAccountType() == BankAccountTypeEnum.PUBLIC_ACCOUNTS
					.getValue()) || (!"305".equals(remitProcess.getBankChannelNo().substring(0, 3)))) { // 该接口的批量打款，只支持民生行内对公打款和跨行打款；
	...
	private ByteArrayOutputStream cmbcInPublicAndOutExcel(List<RemitProcess> inPublicAndOutList, RemitChannel remitChannel ) throws Exception {
		...
		try {
			...
			for (int i = 0, j = 4; i < inPublicAndOutList.size(); i++) {
				...
				if ("305".equals(remitProcess.getBankChannelNo().substring(0, 3))) { // ordertype制单类型:1-行内转帐（只允许对公）2-跨行转帐
																						// （对私或对公）
				...
				if (!"305".equals(remitProcess.getBankChannelNo().substring(0, 3))) { // payeebranchname
				...
				if (!"305".equals(remitProcess.getBankChannelNo().substring(0, 3))) { // pathcode
					...
					if (remitProcess.getProvince().contains("上海")) { // 上海地区走“上海同城支付”；
						if ("403".equals(remitProcess.getBankChannelNo().substring(0, 3))) { // 邮储银行
				...
				if (!"305".equals(remitProcess.getBankChannelNo().substring(0, 3))) { // bnkcode联行号（行内不允许填写，跨行必输）
